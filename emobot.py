...
async def cuteness(ctx, member: discord.Member = None):
    ...
    await ctx.send(
        f"- 🙂 - this is computer's choice don't feel bad if result are bad\n"
        f"📊 {target.display_name} is {num}% {text} 🌸"
    )
...
async def luck(ctx, member: discord.Member = None):
    ...
    await ctx.send(
        f"- 🙂 - this is computer's choice don't feel sad if result are bad\n"
        f"📊 {target.display_name} is {num}% lucky 🍀"
    )       
...
async def profile(ctx):
    ...
    # ---------------- text ----------------
    try:
        font = ImageFont.truetype("Roboto_Condensed-Regular.ttf", 30)
        big_font = ImageFont.truetype("Roboto_Condensed-Regular.ttf", 70)
    except Exception as e:
        print("FONT ERROR:", e)
...
async def add(ctx,val:int,member:discord.Member,*,message=None):
    ...
    send_to = member.id
    if user == 1463873853892857856 or user == 1486400095317659880:
        if not coins.user_exists(send_to):
            await ctx.send("🌸")
            return 
        data = coins.get_user(send_to)
        new_cash = data[1] + val
        coins.update_coins(send_to,new_cash)
        if message == None :
